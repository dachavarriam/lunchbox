...
test("renders the branded Pipiro prototype", async () => {
  ...
  assert.match(html, /Menú permanente/);
  assert.match(html, /Especialidades/);
  assert.match(html, /Chilaquiles/);
  assert.match(html, /Tacos Birria/);
...
test("ships the local PWA assets and pnpm policy", async () => {
  ...
  assert.match(serviceWorker, /pipiro-prototype-v2/);
